...
export default function groupFiltersReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case SET_GROUP_FILTERS:
      ...
      return {...state, group_filters: {...action.payload, ...state.group_filters}}
...
const getGroupFilters = state => Object.values(state.groupFiltersReducer.group_filters);
const getSelectedHubRegion = state => state.inputFieldReducers.hubRegionField;
const getSelectedHubRegionForFilter = state => state.hubSelectionFieldReducer.hubSelected;


// This is for validation setup selector
export const selectedHubFilters = createSelector(
  ...
  (groupFilters, selectedHubRegion) =>{
    ...
    const data = groupFilters.reduce((allFilters, curFilter) => {
      return curFilter.hub_region == selectedHubRegion ? [...allFilters, curFilter] : allFilters
    }, []);
...
export const unSelectedHubFilters = createSelector(
  [getGroupFilters, getSelectedHubRegion], 
  (groupFilters, selectedHubRegion) => groupFilters.filter(filter => filter.hub_region != selectedHubRegion)
);

// This is for the home page selector
